(() => {
  ...
  const emptyMembers = () => Array.from({length:3},()=>({name:"",roles:[]}));
  const emptyWritingSet = () => ({news:{},communication:{},feature:{},commentary:{}});
  const freshReporting = () => ({
    selectedStories:{news:null,communication:null,feature:null,commentary:null},
    acquiredMaterialIds:[], communicationActions:[], featureFollowUp:null,
    commentaryActions:[], writingPlans:emptyWritingSet(),
    drafts:{news:null,communication:null,feature:null,commentary:null},
    unlockedStoryIds:[]
  });
  const freshState = () => ({
    ...
    middayBulletin:null, middayBulletinVersions:[], finalEdition:null, discardedStories:[],
  ...
  function normalizeReporting(reporting={}) {
    ...
    const selectedStories={...base.selectedStories,...(reporting.selectedStories||{})};
    ...
    const drafts={...base.drafts,...(reporting.drafts||{})};
    const writingPlans={...base.writingPlans,...(reporting.writingPlans||{})};
    return {
  ...
  function normalizeNewsroomProfile(profile={}) {
    ...
    /* 兼容上一版5个固定岗位：最多保留3名成员，溢出的职责并入最接近的编辑成员。 */
    const migrated=[];
    const addLegacy=(name,role)=>{
      if(typeof name!=="string"||!name.trim())return;
      const clean=name.trim();
      let member=migrated.find(item=>item.name===clean);
      if(!member&&migrated.length<3){member={name:clean,roles:[]};migrated.push(member);}
    ...
    return {name:typeof profile.name==="string"?profile.name:"",members:[...migrated,...emptyMembers()].slice(0,3)};
  }
  ...
  function allReportingDraftsSubmitted() { return Object.values(newsroomState.reporting.drafts).every(draft=>draft?.submitted); }
  ...
  function goBack() {
    ...
    /* 新闻日前半段近似线性；采写室以后是树状工作台。返回始终回到明确的上一级，
       不再把学生带着沿完整 sceneHistory 一页页倒带。 */
    const parent={briefing:"cover",round1:"briefing",meeting1:"round1",transition2:"meeting1",round2:"transition2",meeting2:"round2",midday:"meeting2",transition3:"midday",round3:"transition3",deadline:"round3",reportingIntro:"deadline",reportingSelect:"reportingIntro",reportingCommunication:"reportingSelect",reportingFeature:"reportingSelect",reportingCommentary:"reportingSelect",writingNews:"reportingSelect",writingCommunication:"reportingCommunication",writingFeature:"reportingFeature",writingCommentary:"reportingCommentary",edition:"reportingSelect",published:"edition",review:"published",bulletinVersion:"round3"};
    const target=parent[newsroomState.currentScene]||"cover";
    newsroomState.previousScene=newsroomState.currentScene;
  ...
  function wireFilterButton(id,label,symbol="") {
    ...
    return `<button type="button" data-wire-filter="${id}" class="${newsroomState.wireFilter===id?"active":""}">${symbol}${label}<b>${count}</b></button>`;
  }
  ...
  function reportingPlanFeasibility() {
    const candidates={};
    Object.keys(REPORTING_GENRES).forEach(genre=>{candidates[genre]=availableReportingStories().filter(story=>story.genres.includes(genre));});
    ...
    if(!missingGenres.length){
      outer: for(const n of candidates.news)for(const c of candidates.communication)for(const f of candidates.feature)for(const o of candidates.commentary){
        const ids=[n.id,c.id,f.id,o.id],counts=ids.reduce((acc,id)=>(acc[id]=(acc[id]||0)+1,acc),{});
        if(new Set(ids).size>=3&&Math.max(...Object.values(counts))<=2){validCombination=ids;break outer;}
      }
    }
    const reasons=[];
    if(missingGenres.length)reasons.push(`还缺可用于${missingGenres.join("、")}的已发现故事`);
    else if(!validCombination)reasons.push("当前已解锁故事不足以满足“四篇至少覆盖3个故事”的要求");
    if(newsroomState.discoveredEvents.length<9)reasons.push(`最终版面至少需要9条彼此可分配的已发现线索，目前只有${newsroomState.discoveredEvents.length}条`);
    return {ready:reasons.length===0,reasons,candidates,validCombination};
  }
  function attemptDeadline() {
    const readiness=reportingPlanFeasibility();
    newsroomState.deadlineLocked=true;saveState();go("deadline");
    if(!readiness.ready)toast(`已进入截稿；提示：${readiness.reasons[0]}`);
  }
  ...
  function materialsFor(storyId,genre) { return REPORTING_MATERIALS.filter(item=>item.storyline===storyId&&(!genre||item.genres.includes(genre))); }
  ...
  function ensureAutomaticMaterials() {
    Object.entries(newsroomState.reporting.selectedStories).forEach(([genre,storyId])=>{
      if(!storyId)return;
      materialsFor(storyId,genre).filter(item=>item.autoGenres.includes(genre)).forEach(item=>acquireMaterial(item.id));
    });
  }
  function acquiredMaterials(storyId,genre) {
    return newsroomState.reporting.acquiredMaterialIds.map(reportingMaterial).filter(item=>item&&item.storyline===storyId&&(!genre||item.genres.includes(genre)));
  }
  function isViewpointMaterial(item) { return item.type==="viewpoint"||(item.type==="interview"&&item.autoGenres.includes("commentary")); }
  function communicationCompleteness(storyId) {
    const items=acquiredMaterials(storyId,"communication");
    const interviews=items.filter(item=>item.type==="interview");
    const background=items.filter(item=>["background","data"].includes(item.type));
    const process=storyEvents(storyId).length>=2||items.some(item=>item.process);
    const missing=[];
    if(interviews.length<2)missing.push(`还需要 ${2-interviews.length} 位人物的直接采访`);
    if(!background.length)missing.push("还需要 1 份背景或数据材料");
    if(!process)missing.push("还需要补齐事实发展过程");
    return {complete:!missing.length,missing,interviews:interviews.length,background:background.length,process};
  }
  ...
  function reportingCompleteness(genre,storyId) {
    if(genre==="communication")return communicationCompleteness(storyId);
    if(genre==="feature")return featureCompleteness(storyId);
    if(genre==="commentary")return commentaryCompleteness(storyId);
  ...
  function roundAction() {
    ...
    const readiness=reportingPlanFeasibility();
    return `<div class="deadline-gate ${readiness.ready?"ready":"not-ready"}"><small>${readiness.ready?"后续四文体采写基础已具备":"截稿检查："+html(readiness.reasons[0]||"还需继续发现新闻")}</small><button data-action="deadline" class="desk-next danger">${readiness.ready?"结束采访 · 进入截稿":"仍可截稿并继续"} →</button></div>`;
  }
  ...
  function renderSubmittedMeeting(round,snapshot) {
    const nextScene=round===1?"transition2":"midday";
    app.innerHTML=`${masthead()}<main class="meeting-scene submitted-meeting"><header><span>EDITORIAL MEETING · ${round===1?"09:00":"13:00"}</span><h1>${round===1?"晨间":"午间"}编辑会议记录</h1><p>这份历史快照已经正式提交，只能查看，不能覆盖。</p></header><section class="submitted-record"><span>PERMANENT SNAPSHOT</span><h2>当时的头条候选</h2><h3>${eventById(snapshot.headline)?.title||"—"}</h3><div><b>三条追踪线索</b>${snapshot.tracks.map((id,index)=>`<p><i>0${index+1}</i>${eventById(id)?.title||id}</p>`).join("")}</div>${snapshot.question?`<div><b>当时最想弄清楚的问题</b><p>${snapshot.question}</p></div>`:""}${snapshot.changed?`<div><b>是否改变判断</b><p>${snapshot.changed==="yes"?"是":"否"}${snapshot.reason?`：${snapshot.reason}`:""}</p></div>`:""}${snapshot.evidence?.length?`<div><b>改变判断的新证据</b>${snapshot.evidence.map(id=>`<p>${eventById(id)?.title||id}</p>`).join("")}</div>`:""}<footer><button class="primary-action" data-resume-scene="${nextScene}">继续新闻日 →</button></footer></section></main>`;
  }
  function renderMeeting(round) {
    const submitted=round===1?newsroomState.meeting1Snapshot:newsroomState.meeting2Snapshot;
    if(submitted){renderSubmittedMeeting(round,submitted);return;}
    const first=round===1, previous=newsroomState.meeting1Snapshot || {tracks:[],headline:null,question:"尚未提交09:00记录"};
    app.innerHTML=`${masthead()}<main class="meeting-scene"><header><span>EDITORIAL MEETING · ${round===1?"09:00":"13:00"}</span><h1>${round===1?"晨间":"午间"}编辑会议</h1><p>${first?"从已经发现的线索中，确定目前最值得继续追踪的方向。":"新证据出现后，重新检查上午的判断。修改判断不是失败。"}</p></header><div class="meeting-layout">${!first?`<aside class="previous-snapshot"><span>你们09:00的判断</span><h2>${eventById(previous.headline)?.title||"—"}</h2><p><b>追踪线索</b>${previous.tracks.map(id=>eventById(id)?.title).join("；")}</p><p><b>核心问题</b>${previous.question}</p></aside>`:""}<form id="meetingForm" class="meeting-form"><p class="optional-flow-note">文字说明均为选填；只做选择也可以提交本次编辑会。</p><fieldset><legend>1. 选择最值得继续追踪的3条线索</legend><p>只能从你们已经发现的信息中选择。</p><div class="meeting-options">${selectionOptions()}</div></fieldset><fieldset><legend>2. 从上述3条中选择当前头条候选</legend><div id="headlineChoices" class="headline-choices"><p>先在上方选满三条线索。</p></div></fieldset>${first?`<label class="text-question"><span>3. 我们现在最想弄清楚的问题是什么？</span><textarea name="question" maxlength="120" placeholder="例如：地铁设备故障和降雨有关吗？"></textarea></label>`:`<fieldset class="change-field"><legend>3. 与09:00相比，你们改变判断了吗？</legend><label><input type="radio" name="changed" value="yes"> 是</label><label><input type="radio" name="changed" value="no"> 否</label><div id="changeEvidence" hidden><p>哪些新证据改变了判断？（选择1—3条）</p><div class="evidence-options">${knownEvents().filter(e=>discoveredDuringRound(e,2)).map(e=>`<label><input type="checkbox" name="evidence" value="${e.id}">${e.title}</label>`).join("")}</div><textarea name="reason" maxlength="160" placeholder="为什么？"></textarea></div></fieldset>`}<button class="primary-action" type="submit">提交${round===1?"09:00":"13:00"}编辑会 →</button></form></div></main>`;
    updateHeadlineChoices();
  }
  function updateHeadlineChoices(){const form=$("#meetingForm");if(!form)return;const ids=[...form.querySelectorAll('input[name="tracks"]:checked')].map(i=>i.value);const box=$("#headlineChoices");box.innerHTML=ids.length?ids.map(id=>`<label><input type="radio" name="headline" value="${id}"><span>${eventById(id).title}</span></label>`).join(""):`<p>先在上方选满三条线索。</p>`;}
  ...
  function renderMidday() { if(newsroomState.middayBulletin){const item=newsroomState.middayBulletin;app.innerHTML=`${masthead()}<main class="bulletin-scene submitted-bulletin"><header><span>S CITY DAILY · 13:00</span><h1>《S城日报 · 午间快讯》</h1><p>原始版本已经发布并永久保存。</p></header><article><span>ORIGINAL · ${item.time}</span><h2>${item.title}</h2><p>${item.lead}</p><footer>信息来源：${item.source}</footer></article><button class="primary-action" data-resume-scene="transition3">继续新闻日 →</button></main>`;return;}app.innerHTML=`${masthead()}<main class="bulletin-scene"><header><span>S CITY DAILY · 13:00</span><h1>《S城日报 · 午间快讯》</h1><p>只发布一条。发布前，请确认现有证据是否足以支持这条消息。</p></header><form id="bulletinForm"><p class="optional-flow-note">标题、导语和来源均可留空；系统会沿用所选线索信息。</p><label>选择报道线索<select name="eventId"><option value="">请选择</option>${knownEvents().map(e=>`<option value="${e.id}">${e.title}</option>`).join("")}</select></label><label>标题（选填）<input name="title" maxlength="50" placeholder="留空则使用所选线索标题"></label><label>一句话导语（选填）<textarea name="lead" maxlength="120" placeholder="留空则使用线索摘要"></textarea></label><label>信息来源（选填）<input name="source" maxlength="100" placeholder="留空则沿用线索来源"></label><label class="evidence-check"><input type="checkbox" name="enough"> 我们认为现有证据足以发布（选填）</label><button class="primary-action" type="submit">发布午间快讯 →</button></form></main>`; }
  ...
  function renderDeadline() { app.innerHTML=`<section class="scene transition-scene deadline-scene"><span>17:00 · EDITION CLOSE</span><time>DEADLINE</time><h1>新闻日采访结束。</h1><p>地图调查已经锁定。接下来进入采写室，<br>把今天真正发现的事实发展为四种新闻作品。</p><div>${newsroomState.discoveredEvents.length}<small>已获得线索</small></div><button class="primary-action" data-action="reporting-intro">进入 S CITY REPORTING ROOM →</button></section>`; }
  ...
  function renderReportingDashboard() {
    ...
    return `<section class="reporting-dashboard"><header><div><span class="reporting-kicker">FOUR WRITING DESKS</span><h2>四篇作品进度</h2></div><p>消息不消耗额外深采；其余三类按各自额度获取材料。</p></header><div class="desk-cards">${Object.entries(REPORTING_GENRES).map(([genre,meta])=>{const story=reportingStory(selections[genre]);const draft=newsroomState.reporting.drafts[genre];const completeness=reportingCompleteness(genre,story.id);const ready=genre==="news"||completeness.complete;return `<article class="desk-card ${draft?.submitted?"submitted":""}"><span>${meta.en}</span><h3>${meta.label}</h3><p>${html(story.title)}<br>${draft?.submitted?`已提交 · ${html(draftDisplayTitle(genre,draft))}`:genre==="news"?"使用已发现事实，直接进入写作":ready?"已达到最低写作条件":`材料待补：${html(completeness.missing[0]||"")}`}</p><footer><b>${draft?.submitted?"✓ 正式提交":ready?"READY":"REPORTING"}</b><button data-open-genre="${genre}">${draft?.submitted?"查看作品":genre==="news"?"进入写作":"进入采写"} →</button></footer></article>`;}).join("")}</div></section>`;
  }
  function renderReportingSelect() {
    rememberUnlockedStories();ensureAutomaticMaterials();saveState();
    const selections=newsroomState.reporting.selectedStories;
    app.innerHTML=`${masthead()}<main class="scene reporting-scene"><div class="reporting-wrap">${reportingHeader("四篇作品计划","为消息、通讯、新闻特写和评论分别选择故事。四篇至少覆盖三个不同故事，同一故事最多用于两种文体。","STORY CONFERENCE")}${reportingRule()}<form id="reportingSelectionForm" class="story-selector"><header><div><span class="reporting-kicker">ASSIGNMENT BOARD</span><h2>选择报道方向</h2></div><p>已经正式提交的作品会锁定其选题，返回本页只能查看，不会覆盖历史稿件。</p></header><div class="genre-selection-grid">${Object.entries(REPORTING_GENRES).map(([genre,meta])=>{const draft=newsroomState.reporting.drafts[genre];const selected=selections[genre];return `<article class="genre-select-card ${draft?.submitted?"done":""}"><span>${meta.en}</span><h3>${meta.label}</h3><select name="${genre}" ${draft?.submitted?"disabled":""}><option value="">选择一个已解锁故事</option>${genreStoryOptions(genre,selected)}</select>${draft?.submitted?`<input type="hidden" name="${genre}" value="${selected}">`:""}<p>${selected?`可继续获取：${html(reportingMaterialChannels(reportingStory(selected)).join(" · "))}`:"只显示本组新闻日已经解锁、且可用于该文体的故事。"}</p></article>`;}).join("")}</div><div class="selection-rule"><strong>版面约束：</strong>四篇作品至少涉及3个不同故事；同一故事最多进入2种文体。系统只检查组合规则，不评价选题高低。</div><div class="reporting-actions"><button type="button" class="secondary-action" data-action="reporting-intro">查看已解锁故事</button><button class="primary-action" type="submit">保存作品计划</button></div></form>${renderReportingDashboard()}<div class="reporting-actions"><span class="progress-copy">四篇正式提交后，只进行版面选择与简讯编排，不再重复撰写主稿。</span><button class="primary-action" data-action="reporting-complete" ${Object.values(newsroomState.reporting.drafts).every(draft=>draft?.submitted)?"":"disabled"}>四篇完成 · 进入版面编排 →</button></div></div></main>`;
  }
  ...
  function renderCompleteness(genre,storyId) {
    ...
    let details=[];
    if(genre==="communication")details=[`人物采访 ${status.interviews}/2`,`背景/数据 ${status.background}/1`,`事实过程 ${status.process?"已形成":"待补"}`];
    if(genre==="feature")details=[`现场观察 ${status.observations} 项`,`人物跟随 ${status.follow?status.follow.source.name:"待选择"}`];
    if(genre==="commentary")details=[`事实依据 ${status.factCount}/3`,`不同观点 ${status.viewpoints}/2`,`政策/数据/第三方 ${status.support?"已具备":"待补"}`];
    return `<section class="completeness ${status.complete?"complete":""}"><b>${status.complete?"✓ 已达到最低写作条件":"最低写作条件检查"}</b><ul>${details.map(item=>`<li>${html(item)}</li>`).join("")}${status.missing.map(item=>`<li>${html(item)}</li>`).join("")}</ul></section>`;
  }
  function reportingActionCard(item,genre) {
    const acquired=newsroomState.reporting.acquiredMaterialIds.includes(item.id);
    const selected=genre==="feature"&&newsroomState.reporting.featureFollowUp===item.id;
    const submitted=newsroomState.reporting.drafts[genre]?.submitted;
    const used=genre==="communication"?newsroomState.reporting.communicationActions.length:genre==="commentary"?newsroomState.reporting.commentaryActions.length:newsroomState.reporting.featureFollowUp?1:0;
    const max=genre==="communication"?4:genre==="commentary"?2:1;
    const disabled=submitted||(!acquired&&used>=max)||(genre==="feature"&&Boolean(newsroomState.reporting.featureFollowUp)&&!selected);
    return `<article class="material-action ${acquired||selected?"acquired":""}"><span>${materialTypeLabel(item.type)} · ${html(item.sourceTag)}</span><h3>${html(item.actionLabel)}</h3><p>${acquired||selected?html(item.summary||"材料已收入记者素材夹。"):'执行前不显示采访或查询结果；完成后材料才会进入记者素材夹。'}</p><button data-reporting-action="${item.id}" data-reporting-genre="${genre}" ${disabled?"disabled":""}>${selected?"已选择此人物":acquired?"已收入素材夹":item.actionLabel}<small>${genre==="feature"?"人物跟随 1/1":`深采动作 ${used}/${max}`}</small></button></article>`;
  }
  function renderMaterialWorkspace(genre) {
    ...
    const story=reportingStory(storyId);const status=reportingCompleteness(genre,storyId);
    let candidates=materialsFor(storyId,genre).filter(item=>!item.autoGenres.includes(genre));
    if(genre==="feature")candidates=candidates.filter(item=>item.featureFollow);
    const acquired=acquiredMaterials(storyId,genre);
    const used=genre==="communication"?newsroomState.reporting.communicationActions.length:genre==="commentary"?newsroomState.reporting.commentaryActions.length:newsroomState.reporting.featureFollowUp?1:0;
    const max=genre==="communication"?4:genre==="commentary"?2:1;
    app.innerHTML=`${masthead()}<main class="scene reporting-scene"><div class="reporting-wrap">${reportingHeader(`${REPORTING_GENRES[genre].label}采写台`,story.title,REPORTING_GENRES[genre].en)}${reportingRule()}<section class="reporting-workspace"><article class="reporting-column"><header><span>DISCOVERED FACTS</span><h2>新闻日事实</h2><p>只显示本编辑部当天主动发现的相关线索。</p></header><div class="reporting-scroll">${renderStoryFacts(storyId)}</div></article><article class="reporting-column"><header><span>REPORTING ACTIONS</span><h2>${genre==="feature"?"选择一位人物跟随":"主动获取新材料"}</h2><p>${genre==="communication"?"最多4次独立深采动作。":genre==="commentary"?"基础事实与两种观点已放入素材夹；还可深采2项。":"基础现场已放入素材夹；只能选择一位人物继续跟随。"}</p><div class="quota-meter"><b>${used} / ${max}</b><i><span style="width:${used/max*100}%"></span></i></div></header><div class="reporting-scroll">${candidates.map(item=>reportingActionCard(item,genre)).join("")||`<div class="desk-empty">当前没有更多可执行动作。</div>`}</div>${renderCompleteness(genre,storyId)}</article><aside class="reporting-column clipbook"><header><span>REPORTER'S CLIPBOOK</span><h2>记者素材夹</h2><p>来源性质、可引用内容和使用边界均随卡片保存。</p></header><div class="reporting-scroll">${acquired.map(materialCard).join("")||`<div class="desk-empty">执行深采动作后，材料会进入这里。</div>`}</div></aside></section><div class="reporting-actions"><button class="secondary-action" data-action="reporting-select">返回四篇计划</button><button class="primary-action" data-write-genre="${genre}">${newsroomState.reporting.drafts[genre]?.submitted?"查看已提交作品":status.complete?"进入写作 →":"材料未齐也可先进入写作 →"}</button></div></div></main>`;
  }
  function planningFields(genre,draft) {
    const plan=draft?.planningAnswers||newsroomState.reporting.writingPlans[genre]||{};
    if(genre==="news")return `<div class="planning-board"><span>LEAD DECISION</span><h3>选择一条导语核心事实</h3><p>从本组实际发现的事实中选择1条你认为最应进入导语的核心事实。系统按时间排序，但“最新”不等于“最重要”。</p>${renderStoryFacts(draft?.storyline||newsroomState.reporting.selectedStories.news,{selectable:true,selectedId:draft?.leadFactId||plan.leadFactId})}</div>`;
    if(genre==="communication")return `<div class="planning-board"><span>WRITING PLAN</span><h3>通讯写作计划</h3><label>这篇通讯要写清谁与什么事？<textarea name="planFocus">${html(plan.focus||"")}</textarea></label><label>开头准备从哪个场景或人物进入？<textarea name="planOpening">${html(plan.opening||"")}</textarea></label><label>文章最终要回答什么问题？<textarea name="planQuestion">${html(plan.question||"")}</textarea></label></div>`;
    if(genre==="feature")return `<div class="planning-board"><span>SCENE PLAN</span><h3>新闻特写写作计划</h3><label>你选择聚焦哪个具体时刻？<textarea name="planMoment">${html(plan.moment||"")}</textarea></label><label>哪一项可观察细节将成为文章线索？<textarea name="planDetail">${html(plan.detail||"")}</textarea></label></div>`;
  ...
  function renderWritingReference(genre,storyId) {
    ...
    return `<article class="writing-reference"><header><span>${genre==="feature"?"FIELD NOTE":"REPORTER'S CLIPBOOK"}</span><h2>${genre==="news"?"已发现事实":genre==="feature"?"现场笔记与人物":"可用采写材料"}</h2></header><div class="writing-reference-body">${renderStoryFacts(storyId)}${materials.map(materialCard).join("")}</div></article>`;
  }
  ...
  function renderWriting(genre) {
    ...
    if(!storyId){go("reportingSelect");return;}if(draft?.submitted){renderSubmittedDraft(genre,draft);return;}
    ...
    app.innerHTML=`${masthead()}<main class="scene writing-scene"><div class="reporting-wrap">${reportingHeader(`${meta.label}写作台`,`${story.title} · 建议篇幅 ${meta.limit}`,meta.en)}${reportingRule()}<section class="writing-layout">${renderWritingReference(genre,storyId)}<article class="writing-paper"><header><span>WRITING DESK · AUTOSAVE</span><h2>完成写作计划与正文</h2><p>所有文字均为选填；课堂正式写作时再完成即可。</p></header><form id="writingForm" class="writing-form" data-genre="${genre}" novalidate>${planningFields(genre,saved)}<label>文章标题（选填）<input type="text" name="title" maxlength="70" value="${html(saved.title||"")}" placeholder="可留空，系统将显示故事名称"></label><label>正文（选填）<textarea name="body" maxlength="${max}" placeholder="可留空直接推进；正式课堂写作时再完成">${html(saved.body||"")}</textarea></label><div class="word-meter"><span>建议篇幅 ${meta.limit}</span><strong id="writingCount">${wordCount(saved.body||"")} 字</strong></div><section class="fact-check"><span>FACT CHECK · SUBMISSION DECLARATION</span><h3>提交前事实核对</h3>${FACT_CHECK_ITEMS.map((item,index)=>`<label><input type="checkbox" name="factCheck" value="${index+1}"> ${item}</label>`).join("")}</section><p class="draft-status">输入内容会自动保存到本机；正式提交后形成只读稿件。</p><div class="writing-actions"><button type="button" class="secondary-action" data-save-draft="${genre}">保存草稿</button><button class="primary-action" type="submit">正式提交并继续 →</button></div></form></article></section></div></main>`;
    if(genre==="commentary"){
  ...
  function renderEditionBriefPicker() {
    ...
    return `<section class="edition-briefing"><header><div><span>NEWS BRIEFING</span><h2>今日简讯编排</h2><p>从本组已经掌握的其他新闻中选择6—10条。无需重写，系统会使用已有新闻素材生成简讯。</p></div><strong id="editionBriefCount">已选 0 / 6—10</strong></header><div class="edition-brief-groups">${groups.map(label=>{const items=knownEvents().filter(event=>publishedSectionLabel(event)===label);return items.length?`<section><h3>${label}<small>${items.length}条可选</small></h3><div>${items.map(event=>`<label class="edition-brief-row" data-brief-card="${event.id}"><input type="checkbox" name="editionBrief" value="${event.id}"><span><b><time>${event.publishTime}</time>${html(event.title)}</b><small>${html(event.source)} · ${verificationLabel(event.verificationStatus)}</small></span></label>`).join("")}</div></section>`:"";}).join("")}</div><p class="edition-brief-note">简讯与“决定不报道”的线索不能重复。若简讯与四篇主稿属于同一故事，请由编辑部判断是否仍有独立信息价值。</p></section>`;
  }
  function syncEditionSelections() {
    ...
    const atLimit=finalBriefIds.size>=10;
    briefInputs.forEach(input=>{input.disabled=discardIds.has(input.value)||(atLimit&&!input.checked);input.closest(".edition-brief-row")?.classList.toggle("unavailable",input.disabled&&!input.checked);});
    ...
    if(counter){counter.textContent=`已选 ${finalBriefIds.size} / 6—10`;counter.classList.toggle("ready",finalBriefIds.size>=6&&finalBriefIds.size<=10);}
  }
  function renderEdition() {
    if(!allReportingDraftsSubmitted()){toast("请先正式提交消息、通讯、特写和评论四篇作品");go("reportingSelect");return;}
    const leadCandidates=["news","communication","feature"].map(genre=>[genre,REPORTING_GENRES[genre],newsroomState.reporting.drafts[genre]]);
    app.innerHTML=`${masthead()}<main class="edition-scene"><header><span>THE S CITY DAILY · FINAL EDITION CONFERENCE</span><h1>S城日报 · 最终版面会议</h1><p>${html(newsroomState.newsroomProfile.name||"S城日报城市新闻部")} · ${SIMULATION.date}　四篇主稿已经完成。现在只决定版面层级、今日简讯和哪些线索不刊发，不再重复写正文。</p></header><form id="editionForm"><section class="edition-conference"><header><span>LEAD DECISION</span><h2>哪一篇作品放在今天最重要的位置？</h2><p>从消息、通讯、新闻特写中选择1篇作为首页头条。评论固定进入 OPINION 栏目，不作为事实新闻头条。</p></header><div class="lead-work-grid">${leadCandidates.map(([genre,meta,draft])=>`<label class="lead-work-card"><input type="radio" name="leadGenre" value="${genre}"><span>${meta.en} · ${meta.label}</span><h3>${html(draftDisplayTitle(genre,draft))}</h3><p>${html(reportingStory(draft.storyline)?.title||"")}</p><small>${wordCount(draft.body)}字 · 已正式提交</small></label>`).join("")}</div></section>${renderEditionBriefPicker()}<section class="discard-editor"><header><span>EDITORIAL CHOICE</span><h2>我们决定不报道</h2><p>至少选择3条没有编入今日简讯的已发现线索，并记录原因。</p></header><div class="discard-list">${knownEvents().map(e=>`<label class="discard-row" data-discard-row="${e.id}"><input type="checkbox" name="discard" value="${e.id}"><span>${html(e.title)}</span><select name="reason-${e.id}">${DISCARD_REASONS.map(r=>`<option>${r}</option>`).join("")}</select><input name="note-${e.id}" placeholder="补充说明（可选）"></label>`).join("")}</div></section><button class="publish-button" type="submit">完成版面选择 · 发布《S城日报》</button></form></main>`;
  ...
  function renderLeadPackage() {
    ...
    if(isReportingEdition()){
      const edition=newsroomState.finalEdition,draft=leadReportingDraft(),meta=REPORTING_GENRES[edition.leadGenre],story=reportingStory(draft.storyline);
  ...
  function renderReportingPublication() {
    ...
    const items=Object.entries(REPORTING_GENRES).filter(([genre])=>drafts[genre]?.submitted&&genre!==leadGenre);
  ...
  function renderPublished() {
    ...
    const hasReporting=Object.values(newsroomState.reporting.drafts).some(draft=>draft?.submitted);
    const leadGenre=isReportingEdition()?newsroomState.finalEdition.leadGenre:null;
    const genreNav=hasReporting?Object.entries(REPORTING_GENRES).filter(([genre])=>newsroomState.reporting.drafts[genre]?.submitted&&genre!==leadGenre).map(([genre,meta])=>`<a href="#published-${genre}">${meta.label}</a>`).join(""):"";
    ...
    app.innerHTML=`<main class="published-scene"><header class="newspaper-masthead"><div class="newspaper-mark">SC</div><div class="newspaper-name"><span>THE S CITY DAILY</span><h1>S城日报</h1><p>${SIMULATION.date} · ${SIMULATION.weekday} · 最终版</p></div><div class="newspaper-edition"><b>${html(profile.name||"S城日报城市新闻部")}</b><time>发布 ${html(newsroomState.publishedAt||"")}</time></div></header><nav class="newspaper-nav" aria-label="版面栏目"><a href="#published-lead">今日头条</a>${genreNav}${sectionNav}</nav>${renderLeadPackage()}${renderTopStories()}${renderReportingPublication()}${renderPublishedSection("S城",city)}${renderPublishedSection("政务",government,{compact:true})}${renderMarketSection(briefEvents)}${renderPublishedSection("全国",national,{compact:true})}${renderPublishedSection("国际",world,{compact:true})}${renderCredits()}<footer class="published-footer"><p>《S城日报》· ${html(profile.name||"城市新闻部")}　发布于 ${html(newsroomState.publishedAt||"")}</p><div><button class="print-action" data-action="print">打印 / 保存为PDF</button><button data-action="review">查看今日编辑部档案 →</button></div></footer></main>`;
  }
  function snapshotBlock(time,snapshot) { if(!snapshot)return `<section class="review-block"><time>${time}</time><div><span>编辑会议</span><h2>本次会议未提交</h2><p>教师导演模式跳过了这一场景，因此没有形成历史快照。</p></div></section>`;return `<section class="review-block"><time>${time}</time><div><span>编辑会议</span><h2>${eventById(snapshot.headline)?.title||"—"}</h2><p><b>追踪：</b>${snapshot.tracks.map(id=>eventById(id)?.title).join("；")}</p>${snapshot.question?`<p><b>核心问题：</b>${snapshot.question}</p>`:""}${snapshot.changed?`<p><b>是否改变：</b>${snapshot.changed==="yes"?"是":"否"}</p>`:""}${snapshot.evidence?.length?`<p><b>改变判断的证据：</b>${snapshot.evidence.map(id=>eventById(id)?.title).join("；")}</p>`:""}</div></section>`; }
  ...
  function renderReview() {
    ...
    const editionSummary=isReportingEdition(edition)?"四篇主稿直接进入版面；未重复撰写第二套头条与副稿。":"3篇主笔报道";
    app.innerHTML=`${masthead()}<main class="review-scene"><header><span>NEWSROOM ARCHIVE · ${SIMULATION.date}</span><h1>今日编辑部档案</h1><p>${html(profile.name||"S城日报城市新闻部")} · 回看一天中，证据如何改变判断，选择如何形成版面。</p></header><div class="review-timeline">${snapshotBlock("09:00",newsroomState.meeting1Snapshot)}${snapshotBlock("13:00",newsroomState.meeting2Snapshot)}<section class="review-block"><time>13:10</time><div><span>午间快讯 · ${versions.length}个版本</span>${versions.map((v,i)=>`<article class="version-card"><b>V${i+1} · ${{original:"原始版本",updated:"更新报道",corrected:"发布更正"}[v.type]}</b><h3>${html(v.title)}</h3><p>${html(v.lead)}</p><small>${v.time}｜${html(v.source)}</small></article>`).join("")}</div></section><section class="review-block"><time>17:00</time><div><span>调查记录</span><h2>全天使用 ${newsroomState.investigationHistory.length} / 8 次采访机会</h2>${newsroomState.investigationHistory.map(h=>`<p>${h.time}　${html(h.action)} → ${html(eventById(h.eventId)?.title||"")}</p>`).join("")||"<p>未执行主动调查。</p>"}</div></section><section class="review-block"><time>EDITION</time><div class="edition-review"><span>最终数字日报</span><h2>${html(profile.name||"S城日报城市新闻部")}</h2>${renderNewsroomMembers("review-newsroom-members")}<p>${editionSummary}</p><div class="authored-review">${authored}</div><div class="edition-review-summary"><strong>额外编入简讯：${getPublishedBriefEvents().length}条</strong>${Object.entries(categoryCounts).map(([label,count])=>`<span>${label} <b>${count}</b></span>`).join("")}</div><h3>我们决定不报道</h3>${newsroomState.discardedStories.map(d=>`<p>${html(eventById(d.id)?.title||"")}｜${html(d.reason)}${d.note?`：${html(d.note)}`:""}</p>`).join("")}</div></section></div><section class="status-review"><span>EDITORIAL STATUS HISTORY</span><h2>线索判断变化</h2><div>${Object.entries(newsroomState.statusHistory).map(([id,history])=>`<article><h3>${html(eventById(id)?.title||"")}</h3><p>${history.map(h=>`${EDITORIAL_STATUS[h.status]?.symbol||""}${EDITORIAL_STATUS[h.status]?.label||h.status}`).join(" → ")}${isLeadEvent(id)?" → HEADLINE":""}</p></article>`).join("")}</div></section><footer><button class="secondary-action" data-action="published">返回最终版面</button><button class="danger-button" data-action="reset">重新开始新闻日</button></footer></main>`;
  }
  function appendReportingReview() {
    const target=$(".status-review");if(!target)return;
  ...
  function render() {
    ...
    if(scene==="cover")renderCover(); else if(scene==="briefing")renderBriefing(); else if(/^round/.test(scene))renderDesk(); else if(scene==="meeting1")renderMeeting(1); else if(scene==="transition2")renderTransition(2); else if(scene==="meeting2")renderMeeting(2); else if(scene==="midday")renderMidday(); else if(scene==="bulletinVersion")renderBulletinVersion(); else if(scene==="transition3")renderTransition(3); else if(scene==="deadline")renderDeadline(); else if(scene==="reportingIntro")renderReportingIntro(); else if(scene==="reportingSelect")renderReportingSelect(); else if(scene==="reportingCommunication")renderMaterialWorkspace("communication"); else if(scene==="reportingFeature")renderMaterialWorkspace("feature"); else if(scene==="reportingCommentary")renderMaterialWorkspace("commentary"); else if(scene==="writingNews")renderWriting("news"); else if(scene==="writingCommunication")renderWriting("communication"); else if(scene==="writingFeature")renderWriting("feature"); else if(scene==="writingCommentary")renderWriting("commentary"); else if(scene==="edition"){if(newsroomState.finalEdition)renderLockedEdition();else renderEdition();} else if(scene==="published")renderPublished(); else if(scene==="review"){renderReview();appendDeskReview();appendReportingReview();} else {newsroomState.currentScene="cover";saveState();renderCover();}
    appendBackButton();
  ...
  function submitMeeting(form,round) {
    if ((round===1 && newsroomState.meeting1Snapshot) || (round===2 && newsroomState.meeting2Snapshot)) { toast("这次编辑会议已提交，历史快照不能修改"); return; }
    ...
    if(round===1){newsroomState.meeting1Snapshot={tracks,headline,question:form.elements.question?.value?.trim()||"",submittedAt:"09:00"};saveState();go("transition2");}
    else {const changed=form.elements.changed?.value||"no",evidence=[...form.querySelectorAll('input[name="evidence"]:checked')].map(i=>i.value).slice(0,3);newsroomState.meeting2Snapshot={tracks,headline,changed,evidence,reason:form.elements.reason?.value.trim()||"",submittedAt:"13:00"};saveState();go("midday");}
  }
  function submitBulletin(form,type="original") {
    if(type==="original"&&newsroomState.middayBulletin){toast("午间快讯原始版本已经保存，请使用更新或更正");return;}
    ...
    const item={type,eventId:fallbackEvent?.id||previous.eventId||null,title:form.elements.title?.value.trim()||previous.title||fallbackEvent?.title||"",lead:form.elements.lead?.value.trim()||previous.lead||fallbackEvent?.content||"",source:form.elements.source?.value.trim()||previous.source||fallbackEvent?.source||"",time:type==="original"?"13:10":"17:00"};
  ...
  function submitProfile(form) {
    ...
    saveState();grantRound(1);go("round1");
  }
  function submitEdition(form) {
    if(newsroomState.finalEdition){toast("最终版面已经正式发布，不能覆盖历史版本");return;}
    if(!allReportingDraftsSubmitted()){toast("请先正式提交四篇新闻作品");return;}
    const leadGenre=form.elements.leadGenre?.value||["news","communication","feature"].find(genre=>newsroomState.reporting.drafts[genre]?.submitted);
    const newsBriefIds=[...form.querySelectorAll('input[name="editionBrief"]:checked')].map(input=>input.value);
    if(newsBriefIds.length<6||newsBriefIds.length>10){toast("今日简讯需要选择6—10条");return;}
    ...
    if(discards.length<3){toast("请至少选择3条未刊发线索并说明原因");return;}
    newsroomState.editionBriefIds=[...newsBriefIds];
    newsroomState.finalEdition={mode:"reporting",leadGenre,publishedDraftGenres:Object.keys(REPORTING_GENRES),newsBriefIds:[...newsBriefIds]};
    newsroomState.discardedStories=discards;
    newsroomState.publishedAt=new Date().toLocaleTimeString("zh-CN",{hour:"2-digit",minute:"2-digit",hour12:false});
    saveState();go("published");
  }
  function submitReportingSelection(form) {
    const formData=new FormData(form);const next={};
    Object.keys(REPORTING_GENRES).forEach(genre=>{next[genre]=formData.get(genre)||newsroomState.reporting.selectedStories[genre]||null;});
    for(const [genre,storyId] of Object.entries(next)){
      const story=reportingStory(storyId);
      if(!story||!isStoryUnlocked(story)||!story.genres.includes(genre)){toast(`请为“${REPORTING_GENRES[genre].label}”选择一个已解锁故事`);return;}
    }
    const ids=Object.values(next);const unique=new Set(ids);
    if(ids.some(id=>ids.filter(value=>value===id).length>2)){toast("同一故事最多用于两种文体");return;}
    if(unique.size<3){toast("四篇文章至少覆盖三个不同故事");return;}
    Object.keys(next).forEach(genre=>{
      const previous=newsroomState.reporting.selectedStories[genre];
      if(previous&&previous!==next[genre]&&!newsroomState.reporting.drafts[genre]?.submitted){
        newsroomState.reporting.drafts[genre]=null;newsroomState.reporting.writingPlans[genre]={};
        if(genre==="communication")newsroomState.reporting.communicationActions=[];
        if(genre==="feature")newsroomState.reporting.featureFollowUp=null;
        if(genre==="commentary")newsroomState.reporting.commentaryActions=[];
      }
      newsroomState.reporting.selectedStories[genre]=next[genre];
    });
    ensureAutomaticMaterials();saveState();renderReportingSelect();toast("四篇作品计划已保存");
  }
  function performReportingAction(id,genre) {
    ...
    if(!item||item.storyline!==storyId||!item.genres.includes(genre)){toast("这项材料不属于当前采写任务");return;}
    if(newsroomState.reporting.drafts[genre]?.submitted){toast("这篇作品已经正式提交，采写记录已锁定");return;}
    if(genre==="feature"){
      ...
      const key=genre==="communication"?"communicationActions":"commentaryActions";
      const max=genre==="communication"?4:2;
  ...
  function selectedMaterialIdsForDraft(genre,storyId) {
    ...
    if(genre==="news")return[];
    if(genre==="feature")return items.filter(item=>item.autoGenres.includes("feature")||item.id===newsroomState.reporting.featureFollowUp).map(item=>item.id);
    return items.map(item=>item.id);
  }
  function writingPlanFromForm(form,genre) {
    const value=name=>form.elements[name]?.value?.trim?.()||"";
    if(genre==="news")return{leadFactId:value("leadFactId")};
    if(genre==="communication")return{focus:value("planFocus"),opening:value("planOpening"),question:value("planQuestion")};
    if(genre==="feature")return{moment:value("planMoment"),detail:value("planDetail")};
  ...
  function saveWritingDraft(form,{submitted=false,quiet=false}={}) {
    const genre=form.dataset.genre;if(!REPORTING_GENRES[genre]||newsroomState.reporting.drafts[genre]?.submitted)return;
    ...
    const draft={genre,storyline:storyId,title:form.elements.title?.value.trim()||"",body:form.elements.body?.value||"",selectedMaterialIds:selectedMaterialIdsForDraft(genre,storyId),planningAnswers:plan,savedAt:new Date().toISOString(),submitted, factCheck:checks};
    if(genre==="news")draft.leadFactId=plan.leadFactId;
    if(genre==="commentary"){draft.stance=plan.stance;draft.counterViewpointId=plan.counterViewpointId;}
    if(submitted)draft.submittedAt=new Date().toISOString();
    newsroomState.reporting.writingPlans[genre]=plan;newsroomState.reporting.drafts[genre]=draft;saveState();
    if(!quiet)toast(submitted?`${REPORTING_GENRES[genre].label}已正式提交`:"草稿已保存到本机");
  }
  ...
  app.addEventListener("submit",event=>{event.preventDefault();if(event.target.id==="profileForm")submitProfile(event.target);if(event.target.id==="meetingForm")submitMeeting(event.target,newsroomState.currentScene==="meeting1"?1:2);if(event.target.id==="bulletinForm")submitBulletin(event.target);if(event.target.id==="bulletinVersionForm")submitBulletin(event.target,event.target.elements.type.value);if(event.target.id==="reportingSelectionForm")submitReportingSelection(event.target);if(event.target.id==="writingForm")submitWriting(event.target);if(event.target.id==="editionForm")submitEdition(event.target);});

  function renderTeacherActions(){const box=$("#teacherActions");if(!box||!TEACHER_MODE)return;box.innerHTML=[{label:"开始09:00",scene:"round1",round:1},{label:"进入09:00编辑会",scene:"meeting1",round:1},{label:"推进至13:00",scene:"round2",round:2},{label:"进入13:00编辑会",scene:"meeting2",round:2},{label:"推进至17:00",scene:"round3",round:3},{label:"进入DEADLINE",scene:"deadline",round:3},{label:"进入采写室",scene:"reportingIntro"}].map(item=>`<button data-teacher-scene="${item.scene}" ${item.round?`data-round="${item.round}"`:""}>${item.label}</button>`).join("");}
